...
if(isset($_COOKIE['name'])||isset($_SESSION['name'])||isset($_COOKIE['teacher'])||isset($_SESSION['teacher'])) {
    ...
    if(isset($_POST['submitDeleteAssignment'])&&!empty($_POST['submitDeleteAssignment'])){
        ...
        $query = mysqli_query($connection, "DELETE FROM lectures WHERE id = '$id'");
        if($query){
            echo 'deleted';
        }
        ...
        $query = mysqli_query($connection, "update lectures set title='$title', date='$deadline', semester='$semester', dept_batch='$batch', description='$desp', lecture_file = '$title" . '.' . "$file_type' where id='$update_id'");
